...
from api.models import (db, Users, Courses, Modules, Lessons, MultimediaResources,
                        Purchases, Achievements, UserPoints, UserProgress, UserAchievements)
...
def validate_active_user():
    # Obtener claims del token (no el identity)
    claims = get_jwt()
    ...
    # Construir user_data desde los claims
    user_data = {
        'user_id': claims.get('user_id'),
        'is_active': claims.get('is_active', False),
        'role': claims.get('role'),
        'is_admin': claims.get('is_admin', False),
        'trial_end_date': claims.get('trial_end_date')
    }
        ...
        if trial_end_date and isinstance(trial_end_date, str) and trial_end_date.strip():
            if 'T' in trial_end_date and '-' in trial_end_date:
                try:
                    ...
                    # Si trial_end no tiene timezone, asumir UTC
                    ...
                    print(
                        f"⚠️ ADVERTENCIA: Formato trial_end_date inválido para usuario {user_data.get('user_id')}: {trial_end_date}")

    user_data['_trial_date_parseable'] = trial_date_parseable
...
def validate_user_role(
    ...
    allowed_roles=None,
    require_admin=False,
    allow_demo=False  # Este parámetro ahora será True para compras
):
...
def _handle_multipart_upload(request, user_id, user):
    ...
    trial_visible = request.form.get(
        "trial_visible", "false").lower() == "true"
        ...
        try:
            ...
            upload_result = cloudinary_service.upload_file(
                ...
                description=descriptions[index -
                                         1] if index - 1 < len(descriptions) else ""
            )
            ...
            media = MultimediaResources(
                ...
                description=descriptions[index - 1] if index -
                1 < len(descriptions) else None,
                order=index
            )
...
def handle_paid_course(course, user_id, user_email):
    ...
    return simple_success_response(response_data, 'Pago preparado exitosamente')
...
def login():
    ...
    row = db.session.execute(db.select(Users).where(
        Users.email == email, Users.is_active == True)).scalar()
...
def get_users():
    ...
    if is_admin:
        total_query = db.session.execute(
            db.select(db.func.count()).select_from(Users)).scalar()

        rows = db.session.execute(db.select(Users).order_by(
            Users.user_id).limit(per_page).offset(offset)).scalars()
...
def module_private(module_id):
    ...
    row = db.session.execute(db.select(Modules).where(
...
def purchases_private():
    ...
    if request.method == 'GET':
        # Los administradores ven todas las compras, los usuarios solo las suyas
        query = db.select(Purchases).order_by(Purchases.purchase_id.desc())

        if not is_admin:
            query = query.where(Purchases.user_id == user_id)
        ...
        return build_pagination_response(
            ...
            page=page,
            per_page=per_page,
            message='Historial de compras recuperado'
        )

    if request.method == 'POST':
        # HELPER: validate_request_json - Validar que envíen el curso a adquirir
        data, error_response, status = validate_request_json(['course_id'])
        if error_response:
            return error_response, status
        ...
        if user_role == 'student' or is_admin:
            # Obtener el email del usuario para Stripe
            user_obj = db.session.get(Users, user_id)
...
def user_achievement(user_achievement_id):
    ...
    if request.method == 'PUT':
        ...
        if 'obtained_date' in data:
            try:
                row.obtained_date = datetime.fromisoformat(
...
def stripe_webhook():
    ...
    try:
        # Cargamos el evento enviado por Stripe
        event = json.loads(payload)
    except Exception as e:
        return {'message': 'Payload inválido'}, 400

    # Extraemos el tipo de evento (ej: payment_intent.succeeded)
    event_type = event.get('type')

    # ESCENARIO: El pago fue exitoso en Stripe
    if event_type == 'payment_intent.succeeded':
        intent = event['data']['object']
        metadata = intent.get('metadata', {})

        # Extraemos los IDs de la metadata (los que enviamos en el purchase-private)
        purchase_id = metadata.get('purchase_id')
        user_id = metadata.get('user_id')
        course_id = metadata.get('course_id')

        if purchase_id:
            # Buscamos la compra específica en la base de datos
            purchase = db.session.get(Purchases, int(purchase_id))

            # Solo procesamos si la compra existe y está pendiente
            if purchase and purchase.status == 'pending':
                # --- 1. ACTIVACIÓN DEL CURSO ---
                purchase.status = 'paid'
                purchase.start_date = datetime.now(timezone.utc)

                # --- 2. ASIGNACIÓN DE PUNTOS (GAMIFICACIÓN) ---
                course = db.session.get(Courses, purchase.course_id)
                user_obj = db.session.get(Users, purchase.user_id)

                if course and user_obj:
                    puntos_a_sumar = course.points or 0

                    # Actualizamos el saldo total del usuario
                    user_obj.current_points = (
                        user_obj.current_points or 0) + puntos_a_sumar

                    # CORRECCIÓN DEL ERROR: Usamos 'course' porque es el valor permitido en el Enum
                    new_points_entry = UserPoints(
                        user_id=user_obj.user_id,
                        points=puntos_a_sumar,
                        point_type='course',
                        event_description=f"Compra del curso: {course.title}",
                        date=datetime.now(timezone.utc)
                    )
                    db.session.add(new_points_entry)

                # Guardamos todos los cambios en la base de datos
                db.session.commit()
                print(
                    f"✅ Compra {purchase_id} activada y puntos asignados al usuario {user_id}.")
            else:
                print(
                    f"⚠️ La compra {purchase_id} no existe o ya fue procesada.")

    # ESCENARIO: El pago fue rechazado o falló
    elif event_type == 'payment_intent.payment_failed':
        intent = event['data']['object']
        purchase_id = intent.get('metadata', {}).get('purchase_id')

        purchase = db.session.get(Purchases, purchase_id)
        if purchase:
            purchase.status = 'cancelled'
            db.session.commit()
            print(f"❌ El pago de la compra {purchase_id} ha fallado.")

    # Stripe exige un 200 OK para confirmar que recibimos el mensaje
    ...
    if not webhook_secret:
        ...
        verification_result = stripe_service.verify_webhook_signature(
            payload, sig_header)
        ...
        if event_type == 'payment_intent.succeeded':
            ...
            if purchase:
                    ...
                    if course and course.points > 0:
                        existing_points = db.session.execute(
                            db.select(UserPoints).where(
                                ...
                                UserPoints.event_description.like(
                        ...
                        if not existing_points:
                            ...
                            user.current_points = (
                                user.current_points or 0) + course.points
                ...
                if purchase_id_from_meta:
                    purchase = db.session.execute(
                        db.select(Purchases).where(
                            Purchases.purchase_id == int(purchase_id_from_meta))
                    ).scalar()
